...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        commandClick(top)
        commandClick(topStart)
        commandClick(topEnd)

        commandClick(center)
        commandClick(centerStart)
        commandClick(centerEnd)

        commandClick(bottom)
        commandClick(bottomStart)
        commandClick(bottomEnd)
    }
    ...
    private fun commandClick(box: ImageView) {
        ...
        box.setOnClickListener {
            if (box.tag == null && !gameEnd) {
                ...
                if (Win(1)) {
                    Toast.makeText(this@MainActivity, "Jogador 1 Ganhou!", Toast.LENGTH_SHORT).show()
                    gameEnd = true
                } else if (Win(2)) {
                    Toast.makeText(this@MainActivity, "JOgador 2 Ganhou!", Toast.LENGTH_SHORT).show()
                    gameEnd = true
                }
    ...
    private fun Win(value: Int): Boolean {
